  ...
  function Renewal(element, configuration) {
    ...
    self.moveTo = function moveTo(position, speed, easing) {
      ...
      if (position >= 0 && position < self.length) {
        ...
        if (conf.moveStyle) {
          style = conf.moveStyle(oldPosition, currentPosition, self.length,
            getItemWidth(1), {left:'-' + getItemWidth(position) + 'px'});
        } else {
          style = {left:'-' + getItemWidth(position) + 'px'};
        }
        element.trigger(conf.eventBeforeMove, [oldPosition, currentPosition]);
        if (duration) {
          element.animate(style, duration, easing || conf.easing,
            function() { element.trigger(conf.eventAfterMove); });
        } else {
          element.css(style);
    ...
    self.getItemWidth = function (upTo) {
      return getItemWidth(upTo);
    };
  }
